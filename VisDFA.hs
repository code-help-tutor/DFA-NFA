...
nfaEdges ts
  = [(q, r, formatLabels xs) | ((q, r), xs) <- zip edges' labels]
  where
    edges' = nub (edges ts)
    labels = map (\(q,r) -> [x | ((q',x),r') <- ts, q'==q, r'==r]) edges'
    formatLabels xs = intercalate "," (labelsToRanges xs)
    -- Group consecutive letters on edge labels.
    labelsToRanges = (map groupToRange) . groupCharacterRuns . sort
    groupToRange g = case g of
      [x] -> [x]
      [x, y] -> [x, ',', y]
      xs -> [head xs, '.', '.', '.', last xs]
    groupCharacterRuns = groupBy' (\x y -> (fromEnum x) + 1 == (fromEnum y))


-- Data.List.groupBy only works properly with equivalence relations.
-- This version from only requires transitivity.
-- https://stackoverflow.com/a/1316392
